...
> 目标：冻结权威导入面，明确哪些旧入口只是 `DEPRECATED_COMPAT`，哪些还属于 `NEEDS_DECISION`。

## 冻结原则

- `CANONICAL` 表示新代码默认必须走的路径
...
|---|---|---|---|---|---|
| semantic parsing | `local_life_agent.semantic.intent_parser.parse_semantic_frame` / `parse_top_intent` | `local_life_agent.semantic.top_intent_router` | `CANONICAL` | 冻结新 import，旧 wrapper 只读 | 语义解析权威入口应回到 `intent_parser.py` |
