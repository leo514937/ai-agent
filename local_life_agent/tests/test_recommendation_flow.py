...
def _tool_names(tool_results: dict) -> list[str]:
...
def test_recommendation_all_tools_go_through_gateway(monkeypatch: pytest.MonkeyPatch):
    shops = [_fake_shop(f"shop_{idx}", f"火锅店{idx}", rating=4.5 - idx * 0.1) for idx in range(1, 5)]
    ...
    assert response.debug is not None
    tool_names = [tool_name for tool_name, _args in recorded]
    assert "search_shops" in tool_names
    assert "get_shop_detail" in tool_names
    assert "check_open_status" in tool_names
...
def test_recommendation_semantic_constraints_should_surface_in_plan_and_ranking():
    ...
    assert "约会" in plan.get("scene_terms", [])
...
def test_recommendation_success_updates_last_recommendation_list_not_current_shop():
    ...
    assert response.debug is not None
    ranked = response.debug.evidence_pack.get("ranking_snapshot", {}).get("ranked", [])
    session_after = response.debug.session_state_after
...
def test_after_recommendation_first_item_reference_works():
    ...
    tool_names = _tool_names(follow_up.debug.tool_results or {})
    assert "get_coupon_list" in tool_names
    assert "search_shops" not in tool_names
...
def test_after_recommendation_this_shop_must_clarify():
    ...
    assert follow_up.debug is not None
    tool_names = _tool_names(follow_up.debug.tool_results or {})
    assert "请提供完整店名" in follow_up.answer_text
    assert "get_coupon_list" not in tool_names
    assert follow_up.debug.session_state_after.get("current_shop") is None
