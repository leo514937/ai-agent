...
class GraphState(TypedDict, total=False):
    ...
    resolved_target: ResolveShopResult | None
    resolve_shop_result: ResolveShopResult | None  # §1 文档字段：target_resolve 直接输出

    # === 执行计划 (Execution Plan) — write: task_plan/facet_planner/comparison_planner, read: plan_validator/tool_execute ===
    execution_plan: ExecutionPlan | None
    validated_plan: ExecutionPlan | None  # §1 文档字段：plan_validator 校验通过后的输出
    ...
    state_update_plan: SessionWriteDirective | None
    ...
    session_state_before: SessionState | None  # §1 文档字段：加载时的会话快照
