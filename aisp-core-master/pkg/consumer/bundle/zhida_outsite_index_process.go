...
func ZhidaOutSiteIndexProcessBundle() cafe.Bundle {
	topicFuncMap := map[string]process.CommonKafkaProcess{
		string(macro.HotCrawler): process.NewZhidaOutSiteIndexProcessor(),
	}

	return stream.NewStreamBundle("ZhidaOutSiteIndexStream",
		stream.WithSource(stream.NewKafkaSource([]string{string(macro.HotCrawler)})),
