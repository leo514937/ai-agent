...
import (
	...
	generate2 "git.in.zhihu.com/zhihu/aisp-core/pkg/business/zhida_agent/graph/logic/generate"
...
// DirectChatSubGraph 直接依靠模型回答 子图(适用 WRU、模型自我介绍、意图澄清、越狱捕捉)
func DirectChatSubGraph[C, U, I any]() *graph.Graph {
	//========图声明========
	var apiConfigPath = zagHandler.GetApiConfigPath()
	var zagGraph = zagHandler.NewZagGraphDeclareWithInfo("direct_chat_sub_graph", entities.BuildGraphScene(graph_constant.ApiStreamChatSub, proto.ChatType_ZHIDA_AGENT.String()), "0.3", "zhoupengcheng", "直答-Router-直接依靠模型回答", apiConfigPath)
	...
	// 生成阶段配置
	var generateConfigStage = zagHandler.NewZagStageDeclare(conf.GenerateConfigStage).AddPres(rootStage)
	...
	var directChatLogic = zagHandler.NewZagLogicDeclare(conf.DirectChatLogic).SetClass(generate2.DirectChatLogic{})
	var streamChatLogic = zagHandler.NewZagLogicDeclare(conf.StreamChatLogic).SetClass(generate.StreamChatLogic{}).
		SetTimeOut(600 * 1000).AddPreLogic(directChatLogic)
	answerGenerateStage.AddLogics(directChatLogic, streamChatLogic)
	...
	//------answer 安全审核--------
	var answerSecurityResultStage = zagHandler.NewZagStageDeclare(conf.AnswerSecurityResultStage)
	...
	answerSecurityResultStage.AddLogics(answerSecurityBeforeUnPass, answerSecurityUnPass)
	...
	//------word 安全审核--------
	var wordSecurityStage = zagHandler.NewZagStageDeclare(conf.WordSecurityStage).AddPres(wordGenerateStage)
	...
	//------响应阶段--------
	var responseStage = zagHandler.NewZagStageDeclare(conf.ResponseStage).AddPres(answerSecurityResultStage, wordSecurityStage)
	...
	stages := []*zagHandler.DeclareStage{
		rootStage,
		generateConfigStage,
		answerGenerateStage, answerSecurityStage, answerSecurityResultStage,
