...
def _format_pack(pack: LocalLifeEvidencePack) -> str:
    lines = [
        f"Query: {pack.query}",
        f"Route: {pack.route}",
        f"Strategy: {pack.retrieval_strategy}",
    ]
    if not pack.parent_evidences:
        lines.append("No evidence found.")
        return "\n".join(lines)
    for index, parent in enumerate(pack.parent_evidences, start=1):
        matched_roles = ", ".join(chunk.chunk_role for chunk in parent.matched_chunks if chunk.chunk_role) or "-"
        sibling_roles = ", ".join(chunk.chunk_role for chunk in parent.sibling_chunks if chunk.chunk_role) or "-"
        lines.append(
            f"[{index}] {parent.shop_name or parent.parent_title} score={parent.parent_score:.2f} parent_id={parent.parent_id}"
        )
        if parent.business_facts:
            business_bits = ", ".join(
                f"{key}={parent.business_facts.get(key)}"
                for key in ("shop_id", "city", "area", "category", "avg_price", "score", "open_hours")
                if parent.business_facts.get(key) not in (None, "")
            )
            lines.append(f"    business_facts: {business_bits or '-'}")
        if parent.parent_context is not None:
            lines.append(
                f"    parent_context: {parent.parent_context.chunk_role or parent.parent_context.chunk_id} -> {parent.parent_context.text[:220]}"
            )
        lines.append(f"    matched_roles: {matched_roles}")
        lines.append(f"    sibling_roles: {sibling_roles}")
        if parent.score_breakdown:
            breakdown = ", ".join(f"{key}={value}" for key, value in parent.score_breakdown.items())
            lines.append(f"    score_breakdown: {breakdown}")
        for chunk in [*parent.matched_chunks, *parent.sibling_chunks]:
            if chunk.text:
                lines.append(f"    - {chunk.chunk_role}: {chunk.text[:220]}")
    return "\n".join(lines)
...
def infer_role_weights(query: str, *, route: str | None = None) -> dict[str, float]:
    ...
    ).infer_role_weights(query, route=route)
