...
from .adapters.helpers import (
    _effective_should_call_tool,
    should_run_tool as routing_should_run_tool,
)
from .adapters.helpers import can_enter_retrieval
from .adapters.helpers import route_execution_mode
from ..routing_registry import resolve_execution_route
...
_RETRIEVAL_ACTIONS = {"recommendation"}
_TOOL_ACTIONS = {"tool_call", "recommendation"}
...
def route_gate(state: GraphState) -> RouteGateCommand:
    ...
    routing = getattr(turn, "routing_decision", None)
    execution_mode = (
        route_execution_mode(routing).execution_mode if routing is not None else "simple"
    )
    ...
    top_level_payload = turn_extra.get("top_level_intent")
    if not isinstance(top_level_payload, Mapping):
        top_level_payload = routing_extra.get("top_level_intent")
    top_level_intent = (
        str(top_level_payload.get("intent") if isinstance(top_level_payload, Mapping) else "")
        .strip()
        .lower()
        or None
    )
    registry_match = resolve_execution_route(
        required_action=str(getattr(routing, "required_action", "") or "").strip().lower()
        if routing is not None
        else None,
        route_candidate=resolved_route_candidate,
        top_level_intent=top_level_intent,
        route_candidates=turn_extra.get("route_candidates")
        if isinstance(turn_extra.get("route_candidates"), list)
        else None,
    )
    registry_route = str(registry_match.get("route_id") or "").strip().lower() or None
    if routing is not None:
        required_action = str(getattr(routing, "required_action", "") or "").strip().lower()
        if execution_mode == "complex":
            branch = "direct"
        elif registry_route:
            branch = registry_route
    ...
    effective_action = effective_action_map.get(
        branch,
    ...
    resolved_top_level_route = (
        str(
            turn_extra.get("resolved_top_level_route")
            or routing_extra.get("resolved_top_level_route")
            or ""
        )
        .strip()
        .lower()
    ...
    if registry_route and execution_mode != "complex":
        branch = registry_route
        effective_action = effective_action_map.get(branch, effective_action)
    required_action = (
        str(getattr(routing, "required_action", "") or "").strip().lower()
        if routing is not None
        else None
    ...
    route_candidate = (
        str(getattr(routing, "route_candidate", "") or "").strip().lower()
        if routing is not None
        else None
    )
    route_reason = (
        str(getattr(routing, "route_reason", "") or "").strip() if routing is not None else ""
    )
    ...
    if target_shop_id is None and isinstance(routing_extra.get("route_review_decision"), Mapping):
        ...
        if isinstance(route_review, Mapping):
            ...
            if target_shop_id is None and isinstance(
                route_review.get("execution_requirements"), Mapping
            ):
    ...
    candidate_shop_ids = []
    raw_candidates = (
        turn_extra.get("candidate_shop_ids") or routing_extra.get("candidate_shop_ids") or []
    )
    ...
    # If no candidates from turn_extra/routing_extra, check persistent.last_candidates
    # for positional pronouns like "第一家", "第二家", etc.
    if not candidate_shop_ids:
        _POSITIONAL_PRONOUNS = ("第一家", "第二家", "第三家", "刚才那家", "上面那家", "刚推荐的")
    ...
    single_shop_mode = not recommendation_mode and (
        target_shop_id is not None
        or bool(turn_extra.get("explicit_query_shop") or turn_extra.get("target_shop_name"))
    )
    ...
    inferred_coupon = any(
        ...
        for token in ("券", "优惠", "领券", "打折", "代金券", "折扣", "有券", "团购")
    )
    inferred_open = any(
        token in compact_query
        for token in ("营业", "开门", "开着", "营业时间", "现在营业吗", "现在开吗", "营业吗")
    )
    inferred_distance = any(
        token in compact_query for token in ("距离", "有多远", "导航", "路线", "怎么走", "怎么去")
    )
    ...
    if len(user_focused_facets) == 1 and user_focused_facets[0] == "coupon":
        ...
        len(
            [
                name
                for name in user_focused_facets
                if name in {"coupon", "open_status", "distance_eta"}
            ]
        )
        > 1
    ):
    ...
    evidence_allowed = bool(
        routing is not None
        and branch == "recommendation"
        and getattr(routing, "should_retrieve", True)
    )
    tool_allowed = _effective_should_call_tool(routing) if routing is not None else True
    ...
    routing_contract = RoutingContract(
        ...
        clarify_reason=str(getattr(routing, "blocked_reason", "") or "")
        if routing is not None
        else None,
    )
    ...
    state["turn"] = turn.model_copy(
        update={"routing_contract": routing_contract, "extra": turn_extra}
    )
...
def _mark_stage(
    ...
    *,
    ...
    turn = state["turn"]
    routing = getattr(turn, "routing_decision", None)
    trace_route_decision = route_decision or (
        routing.required_action if routing is not None else None
    )
    trace_route_reason = route_reason or (
        routing.route_reason if routing is not None else trace_route_decision
    )
    entry = _stage_entry(
        stage,
        status,
        route_decision=trace_route_decision,
        route_reason=trace_route_reason,
        detail=detail,
    )
...
def _ensure_evidence_result(state: GraphState) -> GraphState:
    turn = state["turn"]
    if turn.rag_result is None:
        is_empty = turn.evidence_pack is None or not getattr(turn.evidence_pack, "items", [])
            ...
            update={
                "tool_result": NormalizedToolResult(
                    ...
                    approval_required=bool(
                        getattr(selection, "approval_required", False)
                        or planner_meta.get("approval_required", False)
                    ),
                    approval_status=getattr(selection, "approval_status", None)
                    if selection is not None
                    else planner_meta.get("approval_status"),
                    approval_request=dict(
                        getattr(selection, "approval_request", {})
                        or planner_meta.get("approval_request", {})
                        or {}
                    ),
                    extra={
...
def _tool_stage_detail(turn) -> dict[str, object]:
    ...
    if not selection.should_execute:
        detail["tool_plan_state"] = (
            selection.reason or selection_meta.get("planning_state") or "no_tool_mapping"
        )
        return detail
...
def _finalize_understand_turn(state: GraphState, services: UnderstandTurnServices) -> GraphState:
    ...
    if routing is not None and routing.blocked:
        state = _mark_stage(
            ...
            "blocked",
            route_decision=_route_decision_for_turn(state["turn"]),
            route_reason=str(routing.blocked_reason or "routing_blocked"),
            detail={"blocked_reason": routing.blocked_reason},
        )
        return state
    state = _mark_stage(
        state,
        ...
        route_reason=str(
            (
                state["turn"].routing_decision.route_reason
                if state["turn"].routing_decision
                else None
            )
            or state["turn"].extra.get("route_reason")
            or "routing"
        ),
        detail={
    ...
    routing = state["turn"].routing_decision
    if routing is not None and str(routing.required_action).strip().lower() in {
        "clarify",
        "reject",
        "direct_answer",
        "memory_update",
    ...
    if (
        routing is not None
        and str(routing.required_action).strip().lower() in _RETRIEVAL_ACTIONS
        and state["turn"].retrieval_plan is None
        and can_enter_retrieval(state).allowed
    ):
        state = services.rewrite_query(state)
    return state


def run_evidence_subgraph(state: GraphState, services: EvidenceSubgraphServices) -> GraphState:
    try:
        from .graphs import build_evidence_graph

        compiled_graph = build_evidence_graph(services)
    except Exception:
        compiled_graph = None
    if compiled_graph is not None:
        ...
        if not list(result["turn"].stage_timeline or []):
            if selection is None or not getattr(selection, "should_execute", False):
                result = _mark_stage(
                    ...
                    route_reason=str(
                        result["turn"].extra.get("route_reason")
                        or _route_decision_for_turn(result["turn"])
                    ),
...
def should_run_evidence(state: GraphState) -> bool:
    return False
...
def route_after_understand(state: GraphState) -> str:
    ...
    if routing is not None and (
        routing.blocked or str(routing.required_action).strip().lower() in _DIRECT_ACTIONS
    ):
...
def route_decider(state: GraphState) -> str:
    ...
    coupon_like_query = any(
        token in compact_query_text for token in ("券", "优惠", "代金券", "团购")
    )
    open_like_query = any(
        token in compact_query_text
        for token in ("营业", "开门", "开着", "营业时间", "现在营业吗", "现在开吗", "营业吗")
    )
    distance_like_query = any(
        token in compact_query_text
        for token in ("距离", "有多远", "导航", "路线", "怎么走", "怎么去")
    )
    explicit_target_shop = (
        any(
            str(source or "").strip()
            for source in (
        ...
        or turn_extra.get("selected_shop_id") is not None
        or turn_extra.get("target_shop_id") is not None
    )
    if routing is None:
        ...
        if isinstance(routing_payload, dict):
            ...
            if required_action == "tool_call":
                return "tool"
            if required_action == "recommendation":
            ...
            top_level_intent = (
                str(
                    routing_payload.get("top_level_intent")
                    or routing_payload.get("intent")
                    or routing_extra.get("top_level_intent")
                    or ""
                )
                .strip()
                .lower()
            ...
            registry_match = resolve_execution_route(
                ...
                route_candidate=route_candidate,
                top_level_intent=top_level_intent,
                route_candidates=routing_payload.get("route_candidates")
                if isinstance(routing_payload.get("route_candidates"), list)
                else None,
            )
            registry_route = str(registry_match.get("route_id") or "").strip().lower()
            if registry_route:
                return registry_route
            if route_candidate in {
                "recommendation",
                "local_life.nearby_recommend",
                "nearby_recommend",
        ...
        if coupon_like_query and open_like_query:
            return "tool" if explicit_target_shop else "clarify"
    ...
    if route_review:
        ...
        if current_action in {"tool_call", "recommendation", "direct_answer"}:
            if current_action == "recommendation" and recommended_action in {
                "tool_call",
                "recommendation",
                "direct_answer",
            }:
        ...
        elif current_action == "clarify" and recommended_action in {
            "tool_call",
            "recommendation",
            "direct_answer",
        }:
    ...
    recommendation_mode = bool(
        ...
        or effective_action == "recommendation"
        or route_candidate
        in {
            "recommendation",
            "local_life.nearby_recommend",
    ...
    generic_search_query = any(
        token in compact_query_text
        for token in ("附近", "周边", "推荐", "找个", "搜", "查附近", "有什么")
    )
    explicit_target_shop = (
        any(
            str(source or "").strip()
        ...
        or extra.get("selected_shop_id") is not None
        or extra.get("target_shop_id") is not None
    )
    ...
    if effective_action == "tool_call":
        return "tool"
    if effective_action == "recommendation":
        return "recommendation"
...
def _filter_evidence_by_contract(state: GraphState) -> GraphState:
    ...
    for item in evidence_pack.items:
        ...
        item_shop_id = (
            item.metadata.get("shop_id")
            or item.metadata.get("parent_shop_id")
            or item.metadata.get("entity_shop_id")
        )
        ...
        if (
            contract.target_shop_id is not None
            and item_shop_id is not None
            and item_shop_id != contract.target_shop_id
        ):
    ...
    if len(kept_items) != len(evidence_pack.items):
        ...
        new_pack = evidence_pack.model_copy(
            update={
                "items": kept_items,
        ...
        state["turn"] = turn.model_copy(update={"evidence_pack": new_pack})
        ...
        if quality is not None:
            ...
            state["turn"] = state["turn"].model_copy(
                update={"evidence_quality": new_quality, "routing_decision": routing}
            )
