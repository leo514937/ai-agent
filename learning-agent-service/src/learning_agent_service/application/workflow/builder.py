...
from .graphs import _MAIN_TOPOLOGY
...
class LangGraphWorkflowRunner(SequentialWorkflowRunner):
    ...
    def run_stream(
        ...
        if hasattr(self._graph, "stream"):
            try:
                ...
                yield from super().run_stream(command, persistent_context)
                return
    ...
    def _normalize_emitted_event(event, final_answer: str):
        ...
        effective_answer = (
            final_answer
            or str(payload_dict.get("answer_text") or "").strip()
            or str((payload_dict.get("context") or {}).get("answer_text") or "").strip()
            or ""
        )
        payload_dict["answer_text"] = effective_answer
        metrics = payload_dict.get("metrics")
        if effective_answer and isinstance(metrics, dict):
            answer_quality = dict(metrics.get("answer_quality") or {})
            answer_quality["final_answer"] = effective_answer
...
def write_langgraph_visualizations(output_dir: str | Path | None = None) -> dict[str, str]:
    ...
    files = {
        ...
        "plan_execute_graph.mmd": export_plan_execute_graph_mermaid(),
        ...
        "langgraph_topology.md": "\n".join(
            [
                ...
                "",
                "## Main Graph",
                f"- entry_point: {_MAIN_TOPOLOGY['entry_point']}",
                f"- terminal: {_MAIN_TOPOLOGY['terminal']}",
                "",
                "```mermaid",
                ...
                "",
                "## Plan Execute Graph",
                f"- entry_point: {describe_plan_execute_graph_topology()['entry_point']}",
                "",
                "```mermaid",
                export_plan_execute_graph_mermaid(),
                "```",
...
def _build_langgraph_runner(services: WorkflowServices, checkpointer: Any = None):
    ...
    graph.add_node("rule_review", lambda state: run_rule_review_node(state))
    ...
    graph.add_node("merge_rank_node", lambda state: run_merge_rank_node(state))
    graph.add_node("contract_review", lambda state: run_contract_review_node(state))
    graph.add_node(
        "compose_answer", 
    ...
    graph.add_edge("load_context", "compose_answer")
    graph.add_edge("understand_turn", "plan_execute_subgraph")
    graph.add_edge("understand_turn", "route_gate")
    graph.add_edge("understand_turn", "compose_answer")
    graph.add_edge("understand_turn", "rag_subgraph")
    graph.add_edge("understand_turn", "tool_subgraph")
    graph.add_edge("understand_turn", "rule_review")
    graph.add_edge("rule_review", "route_gate")
    graph.add_edge("route_gate", "compose_answer")
    graph.add_edge("route_gate", "tool_subgraph")
    graph.add_edge("route_gate", "rag_subgraph")
    graph.add_edge("route_gate", "recommendation_subgraph")
    graph.add_edge("route_gate", "plan_execute_subgraph")
    graph.add_edge("recommendation_subgraph", "merge_rank_node")
    graph.add_edge("tool_subgraph", "merge_rank_node")
    graph.add_edge("rag_subgraph", "merge_rank_node")
    graph.add_edge("plan_execute_subgraph", "merge_rank_node")
