...
from learning_agent_service.infrastructure.repositories.memory_trace_repository import (
...
def build_dependencies(settings: Settings | None = None) -> AppDependencies:
    ...
    (
        ...
        local_life_assistant,
        local_life_assistant_status,
    ) = _build_local_life_adapters(
        resolved,
        infra,
    )
...
def _build_query_rewrite_service(
    settings: Settings, infra: InfrastructureClients
) -> QueryRewriteService:
    ...
    return QueryRewriteService(
        policy.query_rewrite, llm_rewriter=llm_rewriter, hyde_rewriter=hyde_rewriter
    )
...
def _build_dense_retriever(
    ...
    if (
        settings.enable_online_dense_retrieval
        ...
        try:
            return QdrantOnlineDenseRetriever(
                ...
                embedding_adapter=_build_openai_embedding_adapter(
                    ...
                    embedding_model_version=getattr(settings.openai, "embedding_model_version", "")
                    or "",
                    redis_runtime=infra.redis,
                    cache_ttl_seconds=getattr(settings.redis, "embedding_cache_ttl_seconds", 0)
                    or 0,
                ),
                fallback=fallback,
                enabled=True,
...
def _build_sparse_retriever(
    ...
    fallback = HeuristicSparseRetriever(
        chunks, parent_child_resolver, filter_builder=filter_builder
    )
...
def _build_metadata_retriever(
    ...
    fallback = HeuristicMetadataRetriever(
        chunks, parent_child_resolver, filter_builder=filter_builder
    )

    if (
        (settings.enable_online_dense_retrieval or settings.enable_online_sparse_retrieval)
        and infra.qdrant is not None
        and infra.openai is not None
    ):
        try:
            return QdrantMetadataRetriever(
                ...
                enabled=True,
                filter_builder=filter_builder,
                scroll_fallback_enabled=settings.enable_online_dense_retrieval
                or settings.enable_online_sparse_retrieval,
                vector_size=infra.qdrant.knowledge_vector_size,
                distance=infra.qdrant.knowledge_distance,
            )
...
def _build_reranker(
    settings: Settings,
) -> HeuristicReranker | RemoteReranker | CrossEncoderReranker:
    ...
    provider = (
        (
            ...
            or ("remote" if settings.enable_remote_reranker else "heuristic")
        )
        .strip()
        .lower()
    )
...
def _memory_fallback_allowed(settings: Settings) -> bool:
    ...
    return bool(
        ...
        and (
            settings.debug
            or environment in {"development", "dev", "local", "test", "testing", "ci"}
        )
    )
...
def _build_memory_deps(
    ...
    profile_projection_store, profile_projection_status = _build_profile_projection_store(
        repositories
    )
...
def _build_memory_trace_repository(
    repositories: RepositoryBundle,
) -> Optional[MemoryTraceRepository]:
...
def _build_local_life_adapters(
    ...
    java_business_status = AdapterStatus(
        ...
        mode="real"
        if java_business_client.enabled
        else ("fallback" if fallback_allowed else "unavailable"),
        ...
        details={
            ...
            "source": "java"
            if java_business_client.enabled
            else ("transaction_store" if fallback_allowed else "unavailable"),
        },
    )
    ...
    assistant_status = AdapterStatus(
        ...
        details={
            ...
            else ("heuristic" if fallback_allowed else "unavailable"),
            "model": assistant.model
            or (infra.openai.default_model if infra.openai is not None else None),
        },
    )
...
def _build_model_gateway(
    ...
    if settings.prefer_real_adapters and infra.openai is not None:
        return (
            OpenAIBackedModelGateway(
                runtime=infra.openai, fallback=fallback, intent_gate=HeuristicIntentGate()
            ),
            AdapterStatus(
                name="model_gateway",
                mode="real",
...
def _build_rag_runtime_details(
    ...
    *,
    ...
    dense_query_capable = bool(
        ...
        and (
            callable(getattr(qdrant_client, "query_points", None))
            or callable(getattr(qdrant_client, "search", None))
        )
    )

    metadata_query_capable = bool(
        qdrant_client is not None and callable(getattr(qdrant_client, "query_points", None))
    )
    ...
    details: dict[str, Any] = {
        ...
        "knowledge_collection": infra.qdrant.local_life_hybrid_collection
        if infra.qdrant is not None
        else None,
    }
...
def _build_rag_orchestrator(
    ...
    if settings.prefer_real_adapters and infra.qdrant is not None:
        ...
        if load_error is not None and not settings.allow_in_memory_fallback:
            raise RuntimeError(
                "Qdrant knowledge runtime is unavailable and in-memory fallback is disabled"
            ) from load_error
    ...
    dense_retriever = _build_dense_retriever(
        settings, infra, active_chunks, parent_child_resolver, filter_builder
    )

    sparse_retriever = _build_sparse_retriever(
        settings, infra, active_chunks, parent_child_resolver, filter_builder
    )

    metadata_retriever = _build_metadata_retriever(
        settings, infra, active_chunks, parent_child_resolver, filter_builder
    )
...
def _build_sparse_query_adapter():
    ...
    class TokenBagSparseQueryAdapter:
        def encode(self, text: str) -> Mapping[str, Any]:
            ...
            return {
                "indices": [_stable_sparse_index(token) for token in tokens],
...
def _point_to_knowledge_chunk(point: Any) -> Optional[KnowledgeChunk]:
    ...
    chunk_id = (
        ...
        or (point.get("id") if isinstance(point, Mapping) else None)
    )
...
def _build_session_context_store(
    ...
    if settings.prefer_real_adapters and infra.redis is not None:
        return (
            ...
            AdapterStatus(
                ...
                details={
                    "backend": "redis",
                    "truth_boundary": "short_term_session",
                    "supports_load_any": True,
                },
            ),
        )
    ...
    if not _memory_fallback_allowed(settings):
        raise RuntimeError(
            "Redis session context store is unavailable and in-memory fallback is disabled"
        )
    ...
    return (
        ...
        AdapterStatus(
            ...
            details={
                "backend": "in_memory",
                "reason": "redis_unavailable_or_disabled",
                "supports_load_any": True,
            },
        ),
    )
...
def _build_async_log_store(
        ...
        return (
            ...
            AdapterStatus(
                ...
                details={
                    ...
                    "path": "outbox",
                    "payload_shape": "typed_or_legacy_mapping",
                },
            ),
        )
    ...
    if not _memory_fallback_allowed(settings):
        raise RuntimeError(
            "Outbox async log store is unavailable and in-memory fallback is disabled"
        )
...
def _build_durable_memory_backend(
    ...
    if not (
        settings.prefer_real_adapters
        and infra.postgres is not None
        and infra.qdrant is not None
        and infra.openai is not None
    ):
        return None
...
def _build_long_term_memory_store(
    ...
    if not _memory_fallback_allowed(settings):
        raise RuntimeError(
            "Long-term memory store is unavailable and in-memory fallback is disabled"
        )
...
def _build_runtime_profile(
    settings: Settings, runtime_status: RuntimeDependencyStatus
) -> RuntimeProfile:
    ...
    elif component_modes and all(
        component.mode == "real" and component.ready for component in component_modes
    ):
        profile_name = "full"
...
def _enrich_local_life_slots(
    raw_intent: Optional[str], slots: Mapping[str, Any], command
) -> dict[str, Any]:
    ...
    scene, preferences = _infer_local_life_scene_preferences(
        command.message, enriched.get("preferences")
    )
...
def _infer_local_life_intent_from_command(command) -> Optional[str]:
    ...
    page = str(
        getattr(command, "page", "")
        or client_context.get("page")
        or client_context.get("entry")
        or ""
    ).lower()

    looks_local_life = page in {
        "meituan_search_box",
        "assistant",
        "ai",
        "shop",
        ...
        key in client_context
        for key in ("shopId", "shopName", "typeId", "typeName", "city", "location")
    )

    if not looks_local_life and not any(
        token in compact
        for token in ("吃饭", "火锅", "餐厅", "店", "优惠券", "团购", "订座", "预约", "订单")
    ):
        return None

    has_coupon = any(token in lowered for token in ("coupon", "voucher", "discount")) or any(
        token in compact for token in ("优惠券", "团购", "券")
    )
    ...
    if any(token in lowered for token in ("coupon", "voucher", "discount")) or any(
        token in compact for token in ("优惠券", "团购", "券")
    ):
        return "coupon"

    if any(token in lowered for token in ("compare", "vs", "difference")) or any(
        token in compact for token in ("对比", "比较", "区别")
    ):
        return "compare"

    if any(token in lowered for token in ("book", "reserve", "booking")) or any(
        token in compact for token in ("订座", "预约", "预订")
    ):
        return "booking"
...
def _infer_local_life_scene_preferences(
    message: Any, existing_preferences: Any
...
def _build_cached_evidence_gate(
    ...
    allowed = normalized_action in {"recommendation", "tool_call"}
    ...
    if not allowed:
        response_kind = route_candidate or (
            ...
            or intent in {IntentType.FOLLOW_UP}
            and confidence < 0.5
            else "empty"
            if confidence <= 0.1
            else "low_info"
